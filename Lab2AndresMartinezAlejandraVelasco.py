...
def kernel(x,y,typ):
    ...
    if typ=='linear':
        ...
        k=math.exp(-numpy.dot(x-y,(x-y).transpose())/(2*sigma**2))
        return k
...
def indicator(x,y):
    ...
    #aux2=numpy.multiply(kernel_xy,targets)    
    return numpy.sum(numpy.multiply(numpy.multiply(kernel_xy,targets),alfa))-be
...
classA = numpy.concatenate((numpy.random.randn(10,2) * 0.6 + [1.5,0.5] ,numpy.random.randn(10,2) * 0.6 + [-1.5 , 0.5 ]))
classB = numpy.random.randn(20,2) * 0.4 + [1.5 ,-0.5]
...
#aux1=numpy.multiply(kernel_sv,targets)   
#aux1=numpy.outer(alfa,targets)
be=numpy.sum(numpy.multiply(numpy.multiply(kernel_sv,targets),alfa))-t_sv[0]
